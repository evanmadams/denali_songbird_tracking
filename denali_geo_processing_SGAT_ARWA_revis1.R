...
#stitch together the data same as with FLightR
...
#this didn't work so hot, so we are going to use a Hill-Ekstrom calibration over winter
...
start = min(which(as.Date(twl[[25]]$Twilight) == startDate))
end = max(which(as.Date(twl[[25]]$Twilight) == endDate))

(zenith_sd[[25]] <- findHEZenith(twl[[25]], tol=0.01, range=c(start,end)))

startDate <- "2018-11-15"
endDate   <- "2019-04-10"

start = min(which(as.Date(twl[[31]]$Twilight) == startDate))
end = max(which(as.Date(twl[[31]]$Twilight) == endDate))

(zenith_sd[[31]] <- findHEZenith(twl[[31]], tol=0.01, range=c(start,end)))

#create preliminary location data based on the HE calibration
...
for(i in c(25, 31)){
  if(nrow(twl[[i]]) > 50){
   ...
   # zenith0[i] <-quantile(z[[i]],prob=0.5)
    ...
    path[[i]] <- thresholdPath(d.twl[[i]]$Twilight, d.twl[[i]]$Rise, zenith=zenith_sd[[i]],tol=0.08)
    na.path[[i]] <- thresholdLocation(d.twl[[i]]$Twilight, d.twl[[i]]$Rise, zenith=zenith_sd[[i]],tol=0.08)
...
#testing custom calibrations to minimize gaps on the wintering grounds during equinox
# 
# #testing a variety of calibrations
# for(i in 25){
#   
#   if(nrow(twl[[i]]) > 50){
#     
#     zenith0[i] <- 91.5  #quantile(z[[i]],prob=0.5)
#     zenith1[i] <- 91.5  #quantile(z[[i]],prob=0.95)
#     
#     d.twl[[i]]<-subset(twl[[i]],twl[[i]]$Twilight>=cal.per[[i]]$calibration.start & !Deleted)
#     path[[i]] <- thresholdPath(d.twl[[i]]$Twilight, d.twl[[i]]$Rise, zenith=zenith0[i],tol=0.08)
#     na.path[[i]] <- thresholdLocation(d.twl[[i]]$Twilight, d.twl[[i]]$Rise, zenith=zenith0[i],tol=0.08)
...
#   if(nrow(twl[[i]]) > 50){
#     
#     #zenith0[i] <- 92.75  #quantile(z[[i]],prob=0.5)
#     #zenith1[i] <- 92.75  #quantile(z[[i]],prob=0.95)
#     
#     zenith0[i] <- 97  #quantile(z[[i]],prob=0.5)
#     zenith1[i] <- 97  #quantile(z[[i]],prob=0.95)
#     
#     d.twl[[i]]<-subset(twl[[i]],twl[[i]]$Twilight>=cal.per[[i]]$calibration.start & !Deleted)
...
for(i in c(25, 31)){
  ...
  if(nrow(twl[[i]]) > 50){
    png(file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$unique.id[i],'prelim_map.png'))
    par(mfrow = c(1, 2), mar = c(2,4,1,1)+0.1)
...
#we run a version of this that restricts species path by range
...
# }
# 
# ranges <- list(fosp, swth, gcth, heth, blpw, arwa, wiwa)
# 
# is.distspp <- list()
# for(i in 1:length(ranges)){
# is.distspp[[i]] <- distribution.mask(shape=ranges[[i]], xlim = c(flr.spp$gridx1[i], flr.spp$gridx2[i]), ylim = c(flr.spp$gridy1[i], flr.spp$gridy2[i]), n = 4, land = T)
# }
# 
...
#restricting movement just based on land generally
...
x0<-z0<-model<-list() #change i back to a sequence when done testing
for(i in c(25, 31)){ 
  if(nrow(twl[[i]]) > 50){
    # Take the location estimates created above
    x0[[i]]<- path[[i]]$x
...
for(i in c(25, 31)){
  if(nrow(twl[[i]]) > 50){
    png(file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$unique.id[i],'refined_map.png'))
    xlim <- xlim.spp[[i]]
    ylim <- ylim.spp[[i]]
    s[[i]] <- locationSummary(fit[[i]]$x,time=model[[i]]$time,collapse=F)
...
for(i in c(25, 31)){
  if(nrow(twl[[i]]) > 50){
    png(file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$unique.id[i],'final_map.png'))
    xlim <- xlimplot.spp[[i]]
    ylim <- ylimplot.spp[[i]]
    s[[i]] <- locationSummary(fit[[i]]$x,time=model[[i]]$time,collapse=F)
    ...
    box()
    dev.off()
    dput(s[[i]], file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$Species[i], unique.recov$unique.id[i],'_locs.out'))
    dput(s.sum[[i]], file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$Species[i], unique.recov$unique.id[i],'_locsum.out'))
    dput(im[[i]], file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$Species[i], unique.recov$unique.id[i],'_image.out'))
    #dput(fit[[i]], file = paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$Species[i], unique.recov$unique.id[i],'_fit.out'))
    
    # #convert to a shapefile
    # tmp.sp <- SpatialPointsDataFrame(cbind(s.sum[[i]]$Lon.mean, s.sum[[i]]$Lat.mean), s.sum[[i]])
    # writeOGR(tmp.sp, dsn = 'P:/Denali NP_critical connections/geolocator data/arwa_revisions', layer = paste0(unique.recov$Species[i], unique.recov$unique.id[i], '_modeled_position'), driver = 'ESRI Shapefile', overwrite = TRUE)
    # 
    #convert to a shapefile
    ...
    writeRaster(r[[i]], paste0('P:/Denali NP_critical connections/geolocator data/arwa_revisions/', unique.recov$Species[i], unique.recov$unique.id[i], '_modeled_space_use_new'), format = 'GTiff', prj = TRUE,  overwrite = TRUE)
...
save.image('denali_geo_060621_arwa_revis.RData')

out <- list()
#pull out the posterior position estimates for each individual
for(i in (1:length(twl))[-27]){
  if(nrow(twl[[i]]) > 50){
    #combine the chains into a data frame
    #out[[i]] <- list(posterior = rbind(fit[[i]]$x[[1]], fit[[i]]$x[[2]], fit[[i]]$x[[3]]), time = fit[[i]]$model$time)
    out[[i]] <- list(posterior = fit[[i]]$x, time = fit[[i]]$model$time, Species = unique.recov$Species[i], ID = unique.recov$unique.id[i])
  }
  }

save(out, file = 'denali_gls_posteriors.Rdata')

#testing for saving ARWA maps properly


### the plan ####


##final anlaysis, a range restricted loop with three iterations after the initial model (also 3 chains and more iterations)

#initial model for 200 iterations

#plot te initial result

#looping for 5000 iterations

#loop another 5000 iterations

#plot the final result

#save results for further mapping and anlaysis
